...
double jacobi(double *h_new, double *h_old, int niters, int energy_intensity, int n, int iter_energy,  const int nsources, int sources[nsources][2], int rank, int size, int px, int py, MPI_Comm comm, int output)
{
    ...
    {
        MPI_Request reqs[8];
        for(int i=0; i<bx; ++i) sbufnorth[i] = h_old[map(i+1,1,bx+2)]; // pack loop - last valid region
        for(int i=0; i<bx; ++i) sbufsouth[i] = h_old[map(i+1,by,bx+2)]; // pack loop
        for(int i=0; i<by; ++i) sbufeast[i] = h_old[map(bx,i+1,bx+2)]; // pack loop
        for(int i=0; i<by; ++i) sbufwest[i] = h_old[map(1,i+1,bx+2)]; // pack loop
        ...
        for(int i=0; i<bx; ++i) h_old[map(i+1,0,bx+2)] = rbufnorth[i]; // unpack loop - into ghost cells
        for(int i=0; i<bx; ++i) h_old[map(i+1,by+1,bx+2)] = rbufsouth[i]; // unpack loop
        for(int i=0; i<by; ++i) h_old[map(bx+1,i+1,bx+2)] = rbufeast[i]; // unpack loop
        for(int i=0; i<by; ++i) h_old[map(0,i+1,bx+2)] = rbufwest[i]; // unpack loop
    ...
    if (output) printarr(h_new, bx, rank);

    MPI_Barrier(comm);
    return calculate_total_heat(h_new, bx);
}
